...
const HrAxisQuestionsForm = memo(
  function HrAxisQuestionsForm({ questions, answers, onAnswerChange, onFileChange, axisId }: Props) {
              ...
              {question.attachment_required && (
                <div className="w-72 shrink-0">
                  <AttachmentInput
                    ...
                    uploadFn={uploadFn}
                    value={
                      (answers?.find((ans) => ans.question_id === question.id)?.attachment as string | undefined) ||
                      null
                    }
                  />
                </div>
  ...
  (prevProps, nextProps) => {
    // Optimize memoization by comparing relevant props
    return (
      prevProps.axisId === nextProps.axisId &&
...
export default HrAxisQuestionsForm;
