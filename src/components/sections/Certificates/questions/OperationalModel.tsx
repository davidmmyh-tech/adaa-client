...
export default function OperationalModel({ onSuccess, isLast }: Props) {
  const [answers, setAnswers] = useState<CertificateAnswer[]>([]);
  const [error, setError] = useState<string | null>(null);
  const { setFlags } = useUserState();

  const { data, isPending } = useQuery({
    queryKey: ['certificate-operational-model-questions'],
    queryFn: () => certificateQuestions('operational')
  });

  const { mutate, isPending: isSubmitting } = useMutation({
    mutationFn: () => submitCertificateQuestions('operational', answers),
    onSuccess: () => {
      onSuccess?.();
      setFlags((prev) => ({ ...prev, completed_operational_certificate: true }));
    },
    onError: (err) => {
      if (isAxiosError(err)) {
  ...
  return (
    <>
      ...
      <div className="flex justify-center">
        {
          <SubmitButton variant="secondary" className="mx-auto w-32" isLoading={isPending} onClick={() => mutate()}>
            {isLast ? 'إنهاء' : 'التالي'}
          </SubmitButton>
        }
      </div>
