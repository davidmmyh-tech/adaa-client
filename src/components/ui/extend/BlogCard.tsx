  ...
  return (
    <div className="flex flex-col gap-6 sm:flex-row" onMouseEnter={() => handlePrefetchBlog(id)}>
      ...
      <div className="w-full space-y-2">
        <Link to={ROUTES.BLOG.DETAILS(id)} className="block text-lg font-semibold">
          {title}
        </Link>
        <p className="text-sm">{description}</p>
        <Link to={ROUTES.BLOG.DETAILS(id)} className="text-secondary mt-4 block text-end text-sm underline">
          اقرأ المزيد
        </Link>
      </div>
    </div>
